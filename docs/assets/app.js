...
const WEBSITE_LOG_ENTRIES = [
  {
    ...
    items: [
      '新增 EP160《新加坡防长陈振声“内病外治”骂的是谁？美国还是中国？东南亚格局将如何演变？》，B 站经接口 is_upower_exclusive=true 核实为会员（充电）视频（金色入口），YouTube 入口正常，两条链接写入 videoLinks 并同步 video-link-overrides，bilibili 标注 access=member，publishedAt 取 B 站 pubdate 2026-06-12；网页标题以 YouTube 主标题为准，去掉【EP160】尾巴。',
      ...
      '新增 3 个关键词词条：陈振声（person，核对现任新加坡国防部长及香会发言立场）、内病外治（concept）、模糊制衡（mechanism）；复用并回填 EP160 的香格里拉对话会、新加坡，并把陈振声、内病外治补进香格里拉对话会的相关关键词。',
      '回填到已有节点：concepts 成本外移、主动中立；models 小国平衡术、规则锚定型小国生存；themes 新加坡与小国生存术、小国平衡与地缘生存、东南亚裂变与集团竞争、大国竞争与战略时间。',
  ...
  },
  {
    ...
    items: [
      ...
      '新增 2 个概念页 科技向善遮羞布（tech-for-good-veneer）、无用阶级（useless-class），1 个思想模型 裁员死亡螺旋（job-elimination-spiral）；回填到已有节点：concepts 成本外移、标准劳动力替代、中产硬着陆、泛体制中产；models 叙事工具化、技术浪潮周期、人机差异化、稳定替代增长陷阱；themes 中产下坠与分层社会、体制化中产与社会流动、平台劳动与真实生存、人的价值与国家信用。',
