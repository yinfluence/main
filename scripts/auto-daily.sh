...
# 由 ~/Library/LaunchAgents/com.yingxiangli.daily-auto.plist 在登录时 + 每天北京时间
# 19:10/19:20/20:10/20:20/23:00/01:00/02:00/03:00 调用（2026-09-15 用户到中国后改为北京时间）。
# 前四个落在扫描窗口里，接住 19 点档和用户要的 20:10 起扫。后四个是整理卡死后的补救通道，
# 也是直播回放的扫描机会。已有实例持锁时后来的触发直接退出，不会重复扫。
